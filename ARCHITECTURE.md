...
| Separate `HceService` entry point | HCE is passive (system calls you on APDU); reader mode is active (cold `Flow`). Different lifecycles and threading models. Android ships now; iOS stubbed until EEA entitlement opens. See [docs/HCE-ARCHITECTURE.md](docs/HCE-ARCHITECTURE.md). |
